report 50005 "NightOwl Sales Cr by Desc VAT"
{
    /* NIGHT0006, Columbus IT, GDC\RAVU, 27 OCT 20
    copy from Custom report 50001
    Added logic to roll up lines and comments to temp table by description.
    -Added new report "Nightowl Sales Cr memo By Desc"

    // RDLCLayout = './Objects/Report/Layout/REP50000.NightOwl Sales Invoice by Work.NOWL.rdlc';

    NIGHT0009, Columbus IT, US\BBR, 01 FEB 21
    Update Billing Reference to print only desc

    NIGHT0010, Columbus IT, US\BBR, 01 MAR 21
        Update for Data Set Changes
    */

    WordLayout = './Objects/Report/Layout/REP50005.NightOwl Sales Credit Memo by Description VAT.NOWL.docx';
    Caption = 'NightOwl Sales Credit Memo for VAT';
    DefaultLayout = Word;
    ...
    dataset
    {
        dataitem(Header; "Sales Cr.Memo Header")
        {
            ...
            column(CompanyFederalID; CompanyInfo."Federal ID No.")
            {

            }
            column(BillingRefferenceNo; "Bill-to Customer No.")
            {

            }
            column(BillingRefferenceName; "Bill-to Name")
            {

            }
            column(BilltoContactName; "Bill-to Contact")
            {
            ...
            column(Your_Reference_2_NOWL; "Your Reference 2.NOWL")
            {
            ...
            column(YourReference2_Header_Lbl; FieldCaption("Your Reference 2.NOWL"))
            {
            }
            ...
            column(OrderComments; TempHeaderComments.Comment)
            {
            ...
            dataitem(Line; "Sales Cr.Memo Line")
            {
                ...
                column(Your_Reference_NOWL_Line; "Your Reference.NOWL")
                {
                }
                column(Your_Reference_2_NOWL_Line; "Your Reference 2.NOWL")
                {

                }
                column(Your_Reference_NOWL_Line_Label; Line.FieldCaption("Your Reference.NOWL"))
                {

                }
                column(Your_Reference_2_NOWL_Line_Label; Line.FieldCaption("Your Reference 2.NOWL"))
                {
                ...
                column(BillingDescription; BillingDescription)
                {

                }
                column(BillingDescLbl; BillingDescLbl)
                {
                ...
                begin
                    // InitializeSalesShipmentLine; // NOWL00006 - Removed as not needed.
                    ...
                    if Quantity <> 0 then
                        FormattedQuantity := Format(Quantity, 0, AutoFormatManagement.ResolveAutoFormat(2, ''));
            ...
            var
                ...
                InvLine: Record "Sales Cr.Memo Line";  // NOWL00006
            ...
            begin
                ...
                IF LineForMatter.FindFirst() then begin
                    if not Matter.GET(LineForMatter."Matter Entry No.NOWL") then
                        Matter.INIT;
                    MatterNoDesc := Matter.Description;  // NIGHT0009 // + '/' + Matter.Number;
                ...
                IF InvLine.FindSet() then
                    repeat
                        ...
                        Line.RESET();
                        Line.SETRANGE("Document No.", InvLine."Document No."); // NIGHT0006
                        Line.SETRANGE(Description, InvLine."Billing Description.NOWL");// NIGHT0006
                        IF Line.FINDFIRST() THEN BEGIN
                            ...
                            Line."Your Reference.NOWL" := InvLine."Your Reference.NOWL";
                            Line."Your Reference 2.NOWL" := InvLine."Your Reference 2.NOWL";
                            Line.MODIFY();
                            LineAdded := TRUE;
                            InsertSalesInvCommentLines(InvLine);// NIGHT0006
                        END ELSE BEGIN
                            Line := InvLine;
                            IF InvLine."No." > '' THEN begin
                                Line.Description := InvLine."Billing Description.NOWL";
                                Line."Description 2" := InvLine."Billing Description.NOWL";// NIGHT0006
                            end;
                            Line.INSERT();
                            LineAdded := TRUE;
                            InsertSalesInvCommentLines(InvLine);// NIGHT0006
                        ...
                        if not LineAdded then begin
                            ...
                            if Resource."Resource Group No." > '' then begin
                                Line.Reset();
                                // Line.SetRange("Resource Group Code.NOWL", Resource."Resource Group No.");// NIGHT0006
                                ...
                                if line.FindFirst() then begin
                                    ...
                                    Line := InvLine;
                                    // Line."Resource Group Code.NOWL" := Resource."Resource Group No.";// NIGHT0006
                                    Line.Insert();
                                    LineAdded := true;
                                end;
                            end;
                            BillingDescription := Resource."Billing Description.NOWL";
                        end;
                        // Rollup for Resource
                        if not LineAdded then begin
                            Line.Reset();
                            ...
                            Line.SetRange("Work Type Code", '');
                            //Line.SetRange("Resource Group Code.NOWL", '');// NIGHT0006
                            if line.FindFirst() then begin
                                line.Quantity += InvLine.Quantity;
                                Line."Line Amount" += InvLine."Line Amount";
                                Line."Inv. Discount Amount" += InvLine."Inv. Discount Amount";
                ...
                // NIGHT0006 S
                EntryNo := 1;
                SalesInvLineComments.Reset();
                SalesInvLineComments.SETRANGE("No.", "No.");
                SalesInvLineComments.SETRANGE("Document Line No.", 0);
                IF SalesInvLineComments.FINDSET THEN
                    REPEAT
                        TempHeaderComments.INIT;
                        TempHeaderComments."Line No." := EntryNo;
                        TempHeaderComments.Comment := SalesInvLineComments.Comment;
                        EntryNo := EntryNo + 1;
                    UNTIL SalesInvLineComments.NEXT = 0;
                // NIGHT0006 E
                TotalSubTotal := 0;
    ...
    var
        ...
        SalesInvLineComments: Record "Sales Comment Line";

        TempHeaderComments: Record "Sales Comment Line" temporary;
        ...
        AmtSubjecttoSalesTaxLbl: Label 'Amount Subject to VAT';    //'Amount Subject to Sales Tax'; // NIGHT0010
        AmtExemptfromSalesTaxLbl: Label 'Amount Exempt from VAT';   //'Amount Exempt from Sales Tax';  // NIGHT0010
        TotalTaxLbl: Label 'Total VAT';  //'Total Tax';   // NIGHT0010
        ...
        LineForMatter: Record "Sales Cr.Memo Line";  // NOWL00006
    ...
    local procedure InsertSalesInvCommentLines(InvoiceLine: Record "Sales Cr.Memo Line")
    var
        IntLineNo: Integer;
    begin
        // NIGHT0006 S
        Line.RESET();
        Line.SETRANGE("Description 2", InvoiceLine."Billing Description.NOWL");
        IF Line.FINDLAST() THEN
            IntLineNo := Line."Line No." + 1;
        // ELSE
        //   IntLineNo := LineNo + 1;
        SalesInvLineComments.RESET();
        SalesInvLineComments.SETRANGE(SalesInvLineComments."Document Type", SalesInvLineComments."Document Type"::"Posted Credit Memo");
        SalesInvLineComments.SETRANGE("No.", InvoiceLine."Document No.");
        SalesInvLineComments.SETRANGE("Document Line No.", InvoiceLine."Line No.");
        IF SalesInvLineComments.FINDSET THEN BEGIN
            REPEAT
                Line.INIT();
                Line."Line No." := IntLineNo;
                Line.Description := SalesInvLineComments.Comment;
                Line."Description 2" := InvoiceLine."Billing Description.NOWL";
                Line.INSERT;
